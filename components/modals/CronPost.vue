...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const response = await axios.get('https://honouree.com/api/v1/cronblogs/fetchBlogsFromMongo');
    ...
    async submitBlog() {
      try {
        const response = await axios.post('https://honouree.com/api/v1/cronblogs/insertBlog', {
    ...
    async deleteBlog(id) {
      try {
        const path = `https://honouree.com/api/v1/cronblogs/deleteBlogsFromMongo/${id}`
