...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        const response = await axios.get('http://13.212.183.88/api/cronblogs/fetchBlogsFromMongo');
    ...
    async submitBlog() {
      try {
        const response = await axios.post('http://13.212.183.88/api/cronblogs/insertBlog', {
    ...
    async deleteBlog(id) {
      try {
        const path = `http://13.212.183.88/api/cronblogs/deleteBlogsFromMongo/${id}`
